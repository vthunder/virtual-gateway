...
export const GW_IP   = [10, 0, 0, 1] as const;
export const MAC_IP  = [10, 0, 0, 2] as const;
export const GW_MAC  = [0xb2, 0x00, 0x00, 0x00, 0x00, 0x01] as const;
export const BCAST_MAC = [0xff, 0xff, 0xff, 0xff, 0xff, 0xff] as const;
...
export function ipToString(b: Uint8Array, o: number): string {
  return `${b[o]}.${b[o+1]}.${b[o+2]}.${b[o+3]}`;
}

export function ipBytesFromString(ip: string): number[] {
...
export function ipChecksum(buf: Uint8Array): number {
  let sum = 0;
...
export function tcpChecksum(srcIp: number[], dstIp: number[], tcpSegment: Uint8Array): number {
...
export function udpChecksum(srcIp: number[], dstIp: number[], udpSegment: Uint8Array): number {
...
export function buildEthernetFrame(
  ...
  for (let i = 0; i < 6; i++) { frame[i] = (dst as number[])[i] ?? 0; }
  for (let i = 0; i < 6; i++) { frame[6 + i] = (src as number[])[i] ?? 0; }
...
export function buildArpReply(arpPayload: Uint8Array): Uint8Array {
  ...
  reply[2] = 8; reply[3] = 0;    // ptype: IPv4
  reply[4] = 6;                   // hlen
  reply[5] = 4;                   // plen
...
export function buildIpTcpPacket(opts: IpTcpOpts): Uint8Array {
...
export function buildIpUdpPacket(opts: IpUdpOpts): Uint8Array {
...
export function buildIcmpPing(srcIp: number[], dstIp: number[], srcMac: Uint8Array): Uint8Array {
...
export function parseDhcpMsgType(bootp: Uint8Array): number | null {
  if (bootp.length < 240) return null;
  for (let i = 0; i < 4; i++) {
    if (bootp[236 + i] !== DHCP_MAGIC[i]) return null;
  }
  let pos = 240;
...
export function buildDhcpReply(bootp: Uint8Array, msgType: number): Uint8Array {
  ...
  let p = 240;
  function opt(code: number, bytes: number[]) {
...
export function buildDnsAReply(dnsQuery: Uint8Array): Uint8Array | null {
  ...
  if (anCount) {
    ...
    rview.setUint16(a, 0xc00c); a += 2; // name ptr
    rview.setUint16(a, 1); a += 2;       // TYPE A
    rview.setUint16(a, 1); a += 2;       // CLASS IN
    rview.setUint32(a, 300); a += 4;     // TTL 300s
    rview.setUint16(a, 4); a += 2;       // RDLENGTH
    GW_IP.forEach((b, i) => { reply[a + i] = b; });
  }
  return reply;
}

export function dnsExtractQname(dns: Uint8Array): string {
